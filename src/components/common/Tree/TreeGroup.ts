...
export const createCatalogTree = (
  layersList: Record<string, unknown>[],
...
const buildParentTreeNode = (
  ...
  const regionPredicate = (
    groupByParams.keys.find((k) => k.name === TOP_LEVEL_GROUP_BY_FIELD) as KeyPredicate
  ).predicate;
    ...
    children: treeData
      ...
      .map((item) => {
        return {
          ...
          expanded,
          children: [
            ...item.items.sort(alphabeticalSort()).map((rec) => {
              return {
                ...rec,
